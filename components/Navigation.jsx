...
export default function Navigation() {
    ...
    return (
        <>
            ...
            <nav className={`lg:flex gap-x-6 transform transition ${isOpen ? 'flex flex-col px-[20%] py-12 z-50 h-screen  gap-y-6  absolute inset-y-0 bg-white right-0  translate-x-0 ' : 'hidden max-md:translate-x-full'}`}>
                {nav.map(({ href, label }) => (
                    ...
                    <div className={` ${isOpen ? 'fixed inset-0 z-30 flex justify-end p-[7%] pt-[20%]' : 'hidden'} `} onClick={() => setOpen(false)}>
            <svg
                    className={`fill-current h-3 w-3 ${isOpen ? "block" : "hidden"}`}
                    viewBox="0 0 20 20"
                    xmlns="http://www.w3.org/2000/svg"
